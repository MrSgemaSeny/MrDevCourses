...
public CourseProgressDto getProgress(Long courseId) {
  ...
  },
  {
    ...
    fullExplanation: 'В MrDeveloperуроки открываются по строгой формуле: `(NOW() - enrolled_at) >= ((day_number - 1) * INTERVAL \'1 day\')`. Расчет выполняется динамически на уровне базы данных и бизнес-логики без фоновых cron-задач. Если урок еще заблокирован, выбрасывается LessonLockedException с датой opensAt.',
...
// src/shared/       -> Reusable UI primitives, helpers, types`,
  ...
  },
  {
    ...
    fullExplanation: 'В MrDeveloperиспользуется строгая темная палитра: `#0a0a0c` (основной фон/сайдбар), `#18181b` (карточки и контейнеры), `#18181b`/`#27272a` (границы), `#e2b340` (акцентный золотой), `#10b981` (изумрудный статус завершения). Стили компилируются на лету через Vite плагин `@tailwindcss/vite`.',
